...
Date: 2026-03-28
Description:
    Computes substitution (S), insertion (I) and deletion (D) error rates
    by aligning trimmed Nanopore reads against reference sequences using edlib.
    Generates a per-barcode summary table with mean and standard error for
    each error type. The table is used to produce the stacked error bar plots
    in the Supplementary Material.

Usage:
    python analysis/stacked_error_analysis.py --mode GPU
    python analysis/stacked_error_analysis.py --mode CPU
...
# Reference FASTA files for each taxonomic group
REFS = {
...
def main():
    parser = argparse.ArgumentParser(
        description="CODIGO 12 — Stacked error profile analysis (S/I/D rates)"
    )
    parser.add_argument("--base-dir", type=Path, default=BASE_DIR,
                        help="Base directory with per-barcode results")
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_DIR,
                        help="Directory where the error table will be saved")
    ...
    output_path = args.output_dir / f"stacked_error_table_{args.mode}.tsv"
    df.to_csv(output_path, sep="\t", index=False)

    logger.info("Stacked error profile analysis completed successfully (mode: %s)", args.mode)
